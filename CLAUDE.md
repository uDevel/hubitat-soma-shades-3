...
**Motion state (`opening`/`closing`) is held through the whole travel**, then finalized. Flow: command entry sets `windowShade=opening|closing` + `state.motorMoving=true` and stashes `state.moveTarget` → intermediate lift reports keep that state → when a commanded move's report reaches `moveTarget`, `handleLiftReport()` finalizes **immediately**; otherwise a `settleWindowShade()` timer (the `settleSeconds` pref, default 3s) is the fallback for uncommanded moves / mid-travel stops / stalls (which have no target to match). Final classification uses a hardcoded 1% tolerance (`CLOSED_AT_OR_BELOW`/`OPEN_AT_OR_ABOVE`) since the encoder motor lands on exact 0/100. The Zigbee Window Covering cluster has **no motion-status attribute** — confirmed against SOMA firmware: `0x0017` is the ZCL `Mode` attribute, not OperationalStatus (Matter-only). So motion direction is inferred from streamed position deltas in `handleLiftReport()`.

**Speed is read-only over Zigbee.** Velocity (`0x0014`) holds the SOMA-app-configured motor speed (0–100%); the driver reports it as `motorSpeed` but cannot set it — ZCL writes ack SUCCESS yet the motor ignores them (verified by travel-time tests). The `readSettings` command (and `configure`) read the device-side config attributes; each has a parse handler that decodes it to the log, with velocity→`motorSpeed`, battery voltage→`batteryVoltage`, and type→`coveringType` also stored as attributes (config status, mode, and lift limits are log-only). "swipe to move" is not exposed over Zigbee at all.
